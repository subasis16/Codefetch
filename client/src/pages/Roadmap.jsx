...
const ProjectCard = ({ project }) => {
  ...
  return (
    <div className="bg-[#121212] border border-white/5 rounded-xl overflow-hidden hover:border-ossium-accent/30 transition-all duration-300">
        ...
        <div className="flex flex-col md:flex-row gap-6 items-start">
          ...
          <div className="flex-1 w-full">
            <div className="flex justify-between items-start mb-2">
              <h3 className="text-xl font-bold text-white max-w-[80%]">{project.title}</h3>
              <button
                className={`p-2 rounded-full border border-white/10 transition-transform duration-300 text-white flex items-center justify-center ${expanded ? 'rotate-180 bg-white/10' : 'hover:bg-white/5'}`}
              >
                <FiChevronDown className="w-5 h-5" />
              </button>
            </div>

            <p className="text-ossium-muted text-sm mb-4 leading-relaxed">{project.description}</p>
      ...
      {expanded && (
        <div className="px-6 pb-8 pt-8 border-t border-white/5 bg-black/20">
